...
$sql = "SELECT name, email, phone, specialization FROM doctors WHERE id = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($password)) {
        ...
        $update_sql = "UPDATE doctors SET name=?, email=?, phone=?, specialization=? WHERE email=?";
        $stmt = $conn->prepare($update_sql);
        $stmt->bind_param("sssssi", $name, $email, $phone, $specialization, $hashed_password, $doctor_id);
    } else {
        $update_sql = "UPDATE doctors SET name=?, email=?, phone=?, specialization=? WHERE id=?";
        $stmt = $conn->prepare($update_sql);
        $stmt->bind_param("ssssi", $name, $email, $phone, $specialization, $doctor_id);
    }

    if ($stmt->execute()) {
        $_SESSION['doctor_name'] = $name; // Update session with new name
